...
Go makes it really easy to create a simple HTTP server, and Clerk makes it simple to authenticate any request. The following examples show how to verify a session and retrieve the corresponding user.
...
Let's see how the middleware functions can be used in action.
...
If you decide to verify the session token on your own, please be mindful of API rate limits. It is recommended to cache your JSON Web Key and invalidate the cache only when the key gets rolled.
